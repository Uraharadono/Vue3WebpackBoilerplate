    ...
    <div class="col-12">
      ...
      <sephiroth-modal v-if="parameters.displayColumnSettingsModal">
        <h3 slot="header" class="modal-title">
          {{ this.settings.culture.columns }}
        </h3>

        <div slot="body">
          <ul>
            <li v-for="column in settings.columns"
      ...
      <SephirothGridFooter v-if="settings.showTopPagination"
                           :settings="settings"
      ...
      <div class="row">
        <div class="col-12 table-responsive">
          <table class="table table-bordered table-condensed table-striped table-hover">
            <thead>
              <SephirothGridHeader :settings="settings"
                                   :parameters="parameters" />
            </thead>

            <tbody>
              <SephirothGridRow v-for="(item, index) in settings.items"
                                :key="item.id"
                                :settings="settings"
                                :parameters="parameters"
                                :item="item"
      ...
      <SephirothGridFooter v-if="settings.showBottomPagination"
                           :settings="settings"
                           :parameters="parameters" />
    </div>
  </div>
...
export default {
    name: "SephirothGrid",
    components: { ComponentLoader, SephirothGridHeader, SephirothGridRow, SephirothGridFooter, SephirothModal },
    props: ['settings'],
    data: function () {
        return {
            parameters: {
    ...
    created: function () {
        ...
        if (this.settings.loadFirstPageAutomatically) {
            ...
            if (this.settings.initialFetchFieldName !== undefined &&
    ...
    methods: {
        ...
        getRequestData: function() {
            ...
            if (this.settings.additionalXhrParams !== undefined)
                if(Array.isArray(this.settings.additionalXhrParams) || this.settings.additionalXhrParams.length)
                {
        ...
        getData: function () {
            ...
            ajax.post(this.settings.url, this.requestData)
                .then(response => {
