...
<template>
  <tr>
    ...
    <td v-for="column in settings.columns" v-if="$parent.displayColumn(column)">
      ...
      <img v-if="column.isImage" :src="getCellValue(column)" onerror="this.onerror = null; this.src='https://upload.wikimedia.org/wikipedia/commons/6/65/No-Image-Placeholder.svg'" height="60">
      ...
      <span v-else-if="column.isRouterLink">
        <router-link class="fake-link" :to="{ path: column.routerLinkPath + getFilePathValue(column) }"> {{ getCellValue(column) }} </router-link>
      </span>

      <div v-for="method in column.methods" v-else-if="column.isMethods" :key="method.buttonClass" class="btn-group" role="group">
        <button
          class="btn btn-sm"
    ...
    methods: {
        ...
        getFilePathValue: function(column) {
            ...
            if (!isNullOrWs(column.preRouterLinkParam))
                ...
                result = this.item[column.routerLinkParam];
