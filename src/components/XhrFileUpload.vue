    ...
    <div class="container">
      ...
      <div>
              ...
              <li v-for="(item, ix) in progressIndicators" :key="ix" class="list-group-item d-flex">
                ...
                <div class="col-10 list-info">
                  ...
                  <template v-if="!item.error">
                    <div>{{ (item.value < 100) ? `Uploading... (${item.value}%)` : 'Uploaded, Processing...' }}</div>
                  </template>
                  <template v-else>
                ...
                <button class="btn btn-outline-success btn-square rounded-pill mt-3 mb-3" @click.prevent="cancelUpload(item)">
                  <span class="btn-icon">
        ...
        <div v-if="completedFiles.length > 0" class="card">
          ...
          <div class="card-body">
            <ul class="list-group">
              <li v-for="(item, ix) in completedFiles" :key="ix" class="list-group-item d-flex" @click="visitUrl(item.downloadPage)">
                <div class="col-1 list-image img-block">
                  <font-awesome-icon :icon="['fa', 'cloud-upload-alt']" size="2x" />
                </div>
                <div class="col-10 list-info">
          ...
          <div class="card-footer align-middle text-center">
            <button v-if="completedFiles.length > 0" type="button" class="btn btn-success w-50" @click.prevent="completedFiles = []">
              Clear All
            </button>
          </div>
        </div>
...
import { debounce, throttle, isNullOrWs } from "@/common/methods";
...
export default {
    ...
    props: {
        ...
        additionalFormParams: { type: Array, default:  () => {return []} },
